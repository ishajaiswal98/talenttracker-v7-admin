...
const UpdateDoctor = (props) => {
  ...
  const loadUser = async () =>{
    const res = await axios.get(`http://backend.softgenics.in/api/doctor/${id}`);
    setUser(res.data)
  ...
  return (
    ...
    <section  data-stellar-background-ratio="3">
        <Container className='mt-5 add-doctor'>
          <h2 className='text-center mt-5'> Update <span style={{color:' #3bc0fd '}}>Doctor</span> Profile</h2>
          <Row className='mt-5'>
          <Col md={5} sm={5} >
            ...
            <Col md={7} sm={7}>
               ...
               <Form className="add-doctor-form"   onSubmit={`http://backend.softgenics.in/api/updateDoctor/${id}`}>
                  <div className='text-center d-inline mb-b'>
                      ...
                      <Row>
                        <Col md={6} sm={6}>
                          ...
                          <Form.Control type="text" className="form-control" name="firstname" placeholder="Full Name"  value={firstname} onChange= {onInputChange} required/>
                        </Col>
